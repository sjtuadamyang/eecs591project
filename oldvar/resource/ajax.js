/*
 * @(#)$Id: script_ajax.js,v 1.3 2011/01/12 14:01:27 cvsroot Exp $
 * 
...
function Ajax(recvType, waitId) {
	...
	aj.waitId = waitId ? AB(waitId) : null;//public
	...
	aj.setWaitId = function(waitid) {
		aj.waitId = typeof waitid == 'object' ? waitid : AB(waitid);
	}
...
function appendscript(src, text, reload, charset) {
	...
	if(reload && AB(id)) {
		AB(id).parentNode.removeChild(AB(id));
	}
	...
	try {
		...
		AB('append_parent').appendChild(scriptNode);
	} catch(e) {}
}
	...
	if(typeof o == 'object' && o.getAttribute) {
		if(o.getAttribute('ajaxtarget')) {
			...
			if(o.getAttribute('ajaxfunc')) {
				...
				_attachEvent(o, ajaxevent, newfunction(RegExp.AB1, RegExp.AB2));
			}
		}
	}
}
...
function ajaxpost(formid, func, timeout) {
	...
	if(ajaxframe == null) {
		...
		AB('append_parent').appendChild(ajaxframe);
	}
	AB(formid).target = ajaxframeid;
	AB(formid).action = AB(formid).action + '&inajax=1';
...
function ajaxpost_load() {
	...
	if(is_ie) {
		var s = AB('ajaxframe').contentWindow.document.XMLDocument.text;
	} else {
		var s = AB('ajaxframe').contentWindow.document.documentElement.firstChild.nodeValue;
	}
	evaled = false;
	if(s.indexOf('ajaxerror') != -1) {
		evalscript(s);
	...
	if(!evaled && AB(formstatus)) {
		AB(formstatus).style.display = '';		
		ajaxinnerhtml(AB(formstatus), s);
		evalscript(s);
	}
...
function ajaxmenu(e, ctrlid, isbox, timeout, func) {
	...
	if(!div) {
		...
		AB('append_parent').appendChild(div);
	}

	var x = new Ajax();
	var href = !isUndefined(AB(ctrlid).href) ? AB(ctrlid).href : AB(ctrlid).attributes['href'].value;
	x.div = div;
	x.etype = e.type;
...
function showloading(display, wating) {
	...
	AB('ajaxwaitid').innerHTML = wating;
	AB('ajaxwaitid').style.display = display;
}
...
function ajaxinnerhtml(showid, s) {
	if(showid.tagName != 'TBODY') {
		...
		AB('append_parent').appendChild(div1);
