 ...
 * @(#)ABId: script_common.js,v 1.5 2011/01/13 11:03:40 cvsroot Exp AB
...
function seccode() {
    ...
    var img = 'do.jsp?ac=seccode&rand='+Math.random();
...
function resizeImg(id,size) {
    ...
    var theImages = AB(id).getElementsByTagName('img');
...
function copyRow(tbody) {
    ...
    if(AB(tbody).rows.length == 1 && AB(tbody).rows[0].style.display == 'none') {

        AB(tbody).rows[0].style.display = '';

        newnode = AB(tbody).rows[0];

    } else {

        newnode = AB(tbody).rows[0].cloneNode(true);
...
function delRow(obj, tbody) {
    ...
    if(AB(tbody).rows.length == 1) {
        ...
        AB(tbody).removeChild(obj.parentNode.parentNode);
...
function checkImage(url) {
    ...
    var re = /^http\:\/\/.{5,200}\.(jpg|gif|png)AB/i
...
function stopMusic(preID, playerID) {
    ...
    if(AB(musicFlash)) {
        ...
        AB(musicFlash).SetVariable('closePlayer', 1);
...
function showFlash(host, flashvar, obj, shareid) {
    ...
    var flashAddr = {
        ...
        'pomoho.com' : 'http://video.pomoho.com/swf/out_player.swf?flvid=FLASHVAR&outall=true',

        'ifeng.com' : 'http://v.ifeng.com/include/exterior.swf?guid=FLASHVAR&AutoPlay=false',
    ...
    if('music' == host) {
        ...
        var mp3Reg = new RegExp('.mp3AB', 'ig');

        var flashReg = new RegExp('.swfAB', 'ig');
    ...
    if(!obj) {
        ...
        AB('flash_div_' + shareid).innerHTML = flashHtml;

        return true;

    }

    if(AB('flash_div_' + shareid)) {

        AB('flash_div_' + shareid).style.display = '';

        AB('flash_hide_' + shareid).style.display = '';
    ...
    if(flashAddr[host]) {
        ...
        if('music' != host&&'flash' != host){
            ...
            if(AB('icon_id_'+shareid)) {

                AB('icon_id_'+shareid).style.display='none';

            }

            AB('media_id_'+shareid).style.display='none';
        ...
        hideObj.onclick = function() {
            ...
            if('music' != host&&'flash' != host){
                ...
                if(AB('icon_id_'+shareid)) {
                    ...
                    AB('icon_id_'+shareid).style.display='';

                }

                AB('media_id_'+shareid).style.display='';
...
function userapp_open() {
    ...
    x.get('cp.jsp?ac=common&op=getuserapp', function(s){

        AB('my_userapp').innerHTML = s;

        AB('a_app_more').className = 'on';

        AB('a_app_more').innerHTML = '收起';

        AB('a_app_more').onclick = function() {
...
function userapp_close() {
    ...
    x.get('cp.jsp?ac=common&op=getuserapp&subop=off', function(s){

        AB('my_userapp').innerHTML = s;

        AB('a_app_more').className = 'off';

        AB('a_app_more').innerHTML = '展开';
...
function showreward() {
    ...
    x.get('do.jsp?ac=ajax&op=getreward', function(s){
...
function msgwin(s, t) {
    ...
    if(!msgWinObj) {
        ...
        AB('append_parent').appendChild(msgWinObj);
...
function displayOpacity(id, n) {
    ...
    if(n >= 0) {
        ...
        AB(id).style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + n + ')';

        AB(id).style.opacity = n / 100;

        setTimeout('displayOpacity(\'' + id + '\',' + n + ')', 50);

    } else {

        AB(id).style.display = 'none';

        AB(id).style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=100)';

        AB(id).style.opacity = 1;
...
function selector(pattern, context) {
    ...
    var s = match[3].replace(/\[(.*)\]/g,'AB1').split('@');

    for(var i=0; i<s.length; i++) {

        var cc = null;

        if (cc = /([\w]+)([=^%!AB~]+)(.*)AB/.exec(s[i])){
        ...
        for(var i=0; i<list.length; i++) {
            ...
            for(var j=0; j<conditions.length; j++) {
                ...
                if(conditions[j][1] == '=') {
                    ...
                    pattern = new RegExp('^'+conditions[j][2]+'AB', 'i');

                } else if(conditions[j][1] == '^=') {

                    pattern = new RegExp('^' + conditions[j][2], 'i');

                } else if(conditions[j][1] == 'AB=') {
