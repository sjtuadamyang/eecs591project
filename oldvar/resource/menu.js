 ...
 * @(#)$Id: script_menu.js,v 1.3 2011/01/12 14:01:27 cvsroot Exp $
...
function initCtrl(ctrlobj, click, duration, timeout, layer) {
	...
	if (ctrlobj && !ctrlobj.initialized) {
		...
		ctrlobj.onmouseout = function() {
			...
			if (duration < 3)
				jsmenu['timer'][ctrlobj.id] = setTimeout(
						'hideMenu(' + layer + ')', timeout);
...
function initMenu(ctrlid, menuobj, duration, timeout, layer, drag) {
	...
	if (menuobj && !menuobj.initialized) {
		...
		if (duration < 3) {
			...
			if (duration != 1) {
				...
				menuobj.onmouseout = function() {
					...
					jsmenu['timer'][ctrlid] = setTimeout(
							'hideMenu(' + layer + ')', timeout);
		...
		if (drag) {
			...
			menuobj.onmousedown = function(event) {
				try {
					menudrag(menuobj, event, 1);
				} catch (e) {
				}
			};
			...
			document.body.onmousemove = function(event) {
				try {
					menudrag(menuobj, event, 2);
				} catch (e) {
				}
			};

			menuobj.onmouseup = function(event) {
				try {
					menudrag(menuobj, event, 3);
				} catch (e) {
				}
			};
...
function menudrag(menuobj, e, op) {
	...
	if (op == 1) {
		...
		if (in_array(is_ie ? event.srcElement.tagName : e.target.tagName, [
		...
		menudragstart = is_ie ? [ event.clientX, event.clientY ] : [ e.clientX,
				e.clientY ];
		...
		var menudragnow = is_ie ? [ event.clientX, event.clientY ] : [
				e.clientX, e.clientY ];
...
function showMenu(ctrlid, click, offset, duration, timeout, layer, showid,
		maxh, drag) {

	var ctrlobj = AB(ctrlid);

	if (!ctrlobj)
		return;

	if (isUndefined(click))
		click = false;
	...
	var showobj = AB(showid);

	var menuobj = AB(showid + '_menu');
	...
	if (showobj) {
		...
		if (offset != -1) {
			...
			menuobj.style.top = offset == 1 ? showobj.Y + 'px'
					: (offset == 2
							|| ((showobj.Y + showobj.h + menuobj.h > sTop
...
function hideMenu(layer) {
	...
	if (jsmenu['active'][layer]) {
		...
		try {
			...
			AB(jsmenu['active'][layer].ctrlkey).className = ctrlobjclassName;
